...
class SinusoidalPositionEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        position_ids = torch.arange(0, n_patches).float().to(x.device)
        indices = torch.arange(0, hidden//2).float().to(x.device)
...
class MambaAffine(nn.Module):
    def __init__(self, config, transform_type='Rigid'):
        super(MambaAffine, self).__init__()
        embed_dim = config["embed_dim"]
        self.transformer = MambaBlock(patch_size=config["patch_size"],
                                      in_chans=config["in_chans"],
                                      embed_dim=config["embed_dim"],
        ...
        if transform_type == 'Similarity' or transform_type == 'Affine':

            scl_out_size = 1 if transform_type == 'Similarity' else 3
            ...
            scl_head_f = nn.Linear(100, scl_out_size)
        ...
        self.inst_norm = nn.InstanceNorm3d(embed_dim * 8)
    ...
    def forward(self, inputs):
        ...
        out = self.transformer(inputs)  # (B, n_patch, hidden)
        out = self.inst_norm(out[-1])
        ...
        if self.transform_type == "Rigid":
            scl = torch.ones((out.shape[0], 3)).to(out.device)
        elif self.transform_type == "Similarity":
            scl = self.scl_mlp(x5)*0.1
            scl = torch.cat([scl, scl, scl], dim=1)
            scl = scl + 1
            scl = torch.clamp(scl, min=1e-4, max=5)
        else:
            scl = self.scl_mlp(x5)*0.1
            scl = scl + 1
            scl = torch.clamp(scl, min=1e-4, max=5)
